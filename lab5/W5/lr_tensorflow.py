...
### define graph
x = tf.placeholder(tf.float32, shape=[None, n_features], name="X")  # None : 데이터 개수
...
### define loss
loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=y)) # tf에서 softmax 쓸 때는 normalize 된 score를 쓰면 안 된다. 속도의 문제 때문.
...
with tf.Session() as sess:
  ...
  ### test
  # calculation for plotting grid, 영역에 색을 칠하는 방법이 없으니 아주 작은 간격으로 점을 찍는다. 그리고 점을 색칠한다.
  h = 0.01
